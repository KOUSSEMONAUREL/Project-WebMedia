...
async function processIgdbOffset(clientId: string, token: string, offset: number, limit: number, db: any, log: ReturnType<typeof createLog>, internalApiUrl: string | null, internalApiKey: string | null): Promise<number> {
    ...
    const inserted: any[] = await db.insert(medias).values(mediaValues).onConflictDoNothing().returning({ id: medias.id, igdbId: medias.igdbId, title: medias.title, slug: medias.slug }) as any;
...
export async function importTrendingGames(clientId: string, clientSecret: string, databaseUrl: string, internalApiUrl: string | null = null, internalApiKey: string | null = null, limit: number = 20) {
    ...
    try {
        ...
        // Freshness pass: always check offset 0 for new top-rated games
        let freshCount = 0;
        if (offset > 0) {
            try {
                freshCount = await processIgdbOffset(clientId, token, 0, limit, db, log, internalApiUrl, internalApiKey);
                if (freshCount > 0) log.info(`Freshness: ${freshCount} new games`);
            } catch (err: any) {
                log.warn(`Freshness pass failed: ${err}`);
            }
        }

        // Deep pass: continue from stored offset
        const deepCount = await processIgdbOffset(clientId, token, offset, limit, db, log, internalApiUrl, internalApiKey);
