...
export class HentaiEraScraper extends BaseScraper {
  readonly name = 'HentaiEra';
  readonly baseUrl = 'https://hentaiera.com';
  readonly lang = 'all';
  ...
  private parseListing(html: string): SearchResult {
    const $ = this.$(html);
    const mangas: Manga[] = $('.gallery_title a, .galleries a').toArray().map(el => {
      const $el = $(el);
      const url = this.absUrl($el.attr('href') || '');
      const img = $el.closest('.galleries').find('img').first();
  ...
  private getInfo($info: cheerio.Cheerio, tag: string): string {
    return $info.find(`li:has(.tags_text:contains(${tag})) .tag .item_name`).toArray().map(el => {
      const $el = $(el);
