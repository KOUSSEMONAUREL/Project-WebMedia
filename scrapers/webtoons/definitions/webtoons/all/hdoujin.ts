...
const BASE_URL = 'https://hdoujin.org';
const BASE_API_URL = 'https://api.hdoujin.org';
const BOOK_API_URL = `${BASE_API_URL}/books`;
...
const PREF_IMAGE_RES = '1280';

export class HdoujinScraper extends BaseScraper {
  readonly name = 'HDoujin';
  ...
  async getPopular(page: number): Promise<SearchResult> {
    ...
    const data = res.data as Entries;
    return {
      mangas: data.entries.map(e => ({
        url: `${e.id}/${e.key}`,
        title: e.title,
        thumbnail: e.thumbnail.path,
      })),
  ...
  async getMangaDetail(mangaUrl: string): Promise<Manga> {
    const res = await this.get(`${BOOK_API_URL}/detail/${mangaUrl}`, this.getHeaders());
  ...
  async getPageList(chapterUrl: string): Promise<Page[]> {
    const res = await this.post(`${BOOK_API_URL}/detail/${chapterUrl}`, this.getHeaders());
    ...
    return imagesInfo.entries.map((image, index) => ({
      index,
      url: `${imagesInfo.base}/${image.path}?w=${PREF_IMAGE_RES}`,
    }));
  }
  ...
  private async getImagesByMangaData(entry: MangaData, entryId: string, entryKey: string): Promise<ImagesInfo> {
    ...
    const imagesRes = await this.get(
      `${BOOK_API_URL}/data/${entryId}/${entryKey}/${id}/${publicKey}/${realQuality}`,
      this.getHeaders(),
    );
  ...
  private detailToManga(detail: MangaDetail): Manga {
    ...
    const genre = [...artists, ...circles, ...parodies, ...characters, ...tags, ...females, ...males, ...mixed, ...other].map(capitalizeEach).join(', ');
