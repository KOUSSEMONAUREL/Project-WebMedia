...
export class HentaiRoxScraper extends BaseScraper {
  readonly name = 'HentaiRox';
  readonly baseUrl = 'https://hentairox.com';
  readonly lang = 'all';
  readonly supportsLatest = true;

  async getPopular(page: number = 1): Promise<SearchResult> {
    const url = page > 1 ? `/top-rated/page/${page}/` : '/top-rated/';
  ...
  private getInfo($info: Cheerio<any>, $: ReturnType<typeof this.$>, tag: string): string {
    return $info.find(`li:has(.tags_text:contains(${tag})) a.tag`).toArray().map(el => {
      const $el = $(el);
      const name = $el.find('.item_name').first().text().trim();
