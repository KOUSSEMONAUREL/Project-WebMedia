...
/**
 * Transcompilation of keiyoushi `fr/hentaiscanreader` (ScanReader multi-src).
 * Same logic as scanreader, different baseUrl/name.
 */
export class HentaiscanreaderScraper extends BaseScraper {
  ...
  async getPopular(page: number = 1): Promise<SearchResult> {
    ...
    const hasNextPage = page > 1 ? $('a.pagination-next').length > 0 : mangas.length > 0;
  ...
  async getMangaDetails(mangaUrl: string): Promise<Partial<Manga>> {
    ...
    $('div.manga-info-grid > div').each((_, el) => {
      ...
      if (label.includes('auteur')) author = valueEl.text().trim();
      else if (label.includes('genres')) genre = valueEl.find('span').toArray().map(s => $(s).text().trim()).filter(Boolean).join(', ');
  ...
  async getChapterList(mangaUrl: string): Promise<Chapter[]> {
    ...
    let html: string;
    try {
      const parsed = JSON.parse(bodyStr) as { data?: string };
    ...
    for (const h4 of $$(`h4`).toArray()) {
      const $h4 = $$(h4);
      const candidate = $h4.parents().toArray().find(p => $$(p).find('a[href*="/chapitre/"]').length > 0);
      let href: string | null = null;
      if (candidate) href = $$(candidate).find('a[href*="/chapitre/"]').first().attr('href') || null;
      if (!href) {
        const direct = $h4.closest('a[href*="/chapitre/"]');
        if (direct.length > 0) href = direct.attr('href') || null;
      }
      if (!href) continue;
      ...
      const scanlator = candidate ? $$(candidate).find('a[title*="Team"]').first().text().trim() || undefined : undefined;
  ...
  private mangaFromCard($el: ReturnType<CheerioAPI>): Manga | null {
    ...
    return { title, url: this.absUrl(href).replace(this.baseUrl, ''), thumbnailUrl: cover ? this.absUrl(cover) : lazy || '', lang: this.lang };
  }
  ...
  private mangaFromLatestCard($cover: ReturnType<CheerioAPI>): Manga | null {
    ...
    return { title, url: this.absUrl(url).replace(this.baseUrl, ''), thumbnailUrl: thumb || '', lang: this.lang };
  }
}
