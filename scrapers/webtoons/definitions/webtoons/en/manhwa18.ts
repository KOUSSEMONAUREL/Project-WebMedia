...
export class Manhwa18Scraper extends BaseScraper {
  readonly name = 'Manhwa18';
  readonly baseUrl = 'https://manhwa18.com';
  readonly lang = 'en';

  async getPopular(page = 1): Promise<SearchResult> {
    const res = await this.get(`${this.baseUrl}/tim-kiem?sort=top&page=$page`);
    const $ = this.$(res.data);
    const mangas: Manga[] = [];
    $("a").each((_, el) => {
      const $el = $(el);
  ...
  async getLatest(page = 1): Promise<SearchResult> {
    const res = await this.get(`${this.baseUrl}/tim-kiem?sort=update&page=$page`);
    const $ = this.$(res.data);
    const mangas: Manga[] = [];
