...
export class NineAnimeScraper extends BaseScraper {
  readonly name = 'NineAnime';
  readonly baseUrl = 'https://www.nineanime.com';
  readonly lang = 'en';

  async getPopular(page = 1): Promise<SearchResult> {
    const res = await this.get(`${this.baseUrl}/category/index_$page.html?sort=views`);
    const $ = this.$(res.data);
    const mangas: Manga[] = [];
    $("a").each((_, el) => {
      const $el = $(el);
  ...
  async getLatest(page = 1): Promise<SearchResult> {
    const res = await this.get(`${this.baseUrl}/category/index_$page.html?sort=updated`);
    const $ = this.$(res.data);
    const mangas: Manga[] = [];
