...
export class EggporncomicsScraper extends BaseScraper {
  readonly name = 'Eggporncomics';
  readonly baseUrl = 'https://eggporncomics.com';
  readonly lang = 'en';

  async getPopular(page = 1): Promise<SearchResult> {
    const res = await this.get(`${this.baseUrl}/category/1/anime-comics?page=$page`);
    const $ = this.$(res.data);
    const mangas: Manga[] = [];
    $("a").each((_, el) => {
      const $el = $(el);
  ...
  async getLatest(page = 1): Promise<SearchResult> {
    const res = await this.get(`${this.baseUrl}/latest-comics?page=$page`);
    const $ = this.$(res.data);
    const mangas: Manga[] = [];
