...
export class DigitalComicMuseumScraper extends BaseScraper {
  readonly name = 'Digital Comic Museum';
  readonly baseUrl = 'https://digitalcomicmuseum.com';
  readonly lang = 'en';

  async getPopular(page = 1): Promise<SearchResult> {
    const res = await this.get(`${this.baseUrl}/stats.php?ACT=topdl&start=${page - 1}00&limit=100`);
    const $ = this.$(res.data);
    const mangas: Manga[] = [];
    $("a").each((_, el) => {
      const $el = $(el);
  ...
  async getLatest(page = 1): Promise<SearchResult> {
    const res = await this.get(`${this.baseUrl}/stats.php?ACT=latest&start=${page - 1}00&limit=100`);
    const $ = this.$(res.data);
    const mangas: Manga[] = [];
