...
export class StatusFilterScraper extends BaseScraper {
  readonly name = 'Manga Demon';
  readonly baseUrl = 'https://demonicscans.org';
  readonly lang = 'en';

  async getPopular(page = 1): Promise<SearchResult> {
    const res = await this.get(`${this.baseUrl}/advanced.php?list=$page&status=all&orderby=VIEWS%20DESC`);
    const $ = this.$(res.data);
    const mangas: Manga[] = [];
    $("a").each((_, el) => {
      const $el = $(el);
  ...
  async getLatest(page = 1): Promise<SearchResult> {
    const res = await this.get(`${this.baseUrl}/lastupdates.php?list=$page`);
    const $ = this.$(res.data);
    const mangas: Manga[] = [];
