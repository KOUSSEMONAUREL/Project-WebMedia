...
export class SchlockmercenaryScraper extends BaseScraper {
  readonly name = 'Schlock Mercenary';
  readonly baseUrl = 'https://www.schlockmercenary.com';
  readonly lang = 'en';

  async getPopular(page = 1): Promise<SearchResult> {
    const res = await this.get(`${this.baseUrl}$ARCHIVE_URL`);
    const $ = this.$(res.data);
    const mangas: Manga[] = [];
    $("a").each((_, el) => {
      const $el = $(el);
